...
const defaultGeneralForm = { submissionRound: '1', semester: '1', academicYear: '2568', curriculumName: '', majorName: '' }
const defaultStudyForm = { studyPlan: 'planB', learningPeriod: 'afterHours', campus: 'chakrabongse' }
...
const isMasterCurriculum = (curriculum) => {
  const levelText = getCurriculumLevelText(curriculum).toLowerCase()
  const nameText = getCurriculumNameThai(curriculum).toLowerCase()
  return levelText === 'master' || levelText.includes('master') || levelText.includes('graduate') || levelText.includes('ปริญญาโท') || levelText.includes('โท') || nameText.includes('มหาบัณฑิต') || nameText.includes('ปริญญาโท')
}
...
function CourseOpeningMasterPage() {
  ...
  const fetchInitialCurriculumAndDepartment = useCallback(async () => {
    ...
    try {
      ...
      if (!userDepartmentId) {
        ...
        setInitialInfoErrorMessage('ไม่พบรหัสสาขาของผู้ใช้งานปัจจุบัน จึงยังไม่สามารถค้นหาหลักสูตรระดับปริญญาโทที่ผูกกับสาขาได้')
        return
      }
      ...
      const matchedCurriculum = curriculumDetailList.find((curriculum) => isMasterCurriculum(curriculum) && isCurriculumMatchedWithDepartment(curriculum, userDepartmentId))
      ...
      if (!matchedCurriculum) setInitialInfoErrorMessage('พบข้อมูลสาขาของผู้ใช้งานแล้ว แต่ยังไม่พบหลักสูตรระดับปริญญาโทที่ผูกกับสาขานี้')
    } catch (error) {
      console.error('Error fetching initial master curriculum and department:', error)
      setInitialInfoErrorMessage(getErrorMessage(error, 'ไม่สามารถดึงข้อมูลหลักสูตรระดับปริญญาโทและสาขาของผู้ใช้งานได้'))
    } finally {
  ...
  const handleChangeStudyForm = (event) => {
    const { name, value } = event.target
    setStudyForm((previousForm) => ({ ...previousForm, [name]: value }))
  }
  ...
  const buildCourseOpeningPayload = () => {
    ...
    return {
      ...
      program_type: studyForm.studyPlan,
      study_plan: studyForm.studyPlan,
      education_level: 'master',
      degree_level: 'master',
      study_mode: studyForm.learningPeriod,
      campus: studyForm.campus,
      target_group: studyForm.studyPlan,
  ...
  return (
    <Box className={styles.page}>
      ...
      <Box className={styles.container}>
        <Box className={styles.pageHeader}>
          <Typography className={styles.pageTitle}>
            {requestData ? 'ดูรายละเอียดการเปิดรายวิชา (ระดับปริญญาโท)' : 'แบบเปิดรายวิชาประจำภาคการศึกษา (ระดับปริญญาโท)'}
          </Typography>

          <Typography className={styles.pageDescription}>
            {requestData ? 'หน้านี้ใช้สำหรับตรวจสอบรายละเอียดเอกสารที่บันทึกไว้ โดยสามารถแก้ไขแบบร่างก่อนส่งเอกสารจริงจากหน้ารายการคำขอเปิดรายวิชา' : 'หน้าสำหรับกรอกข้อมูลคำขอเปิดรายวิชาระดับปริญญาโทและบันทึกเป็นแบบร่างก่อน จากนั้นจึงไปตรวจสอบและกดส่งเอกสารจริงที่หน้ารายการคำขอเปิดรายวิชา'}
          </Typography>
        </Box>
        ...
        <Box className={styles.sectionCard}>
          ...
          {isInitialInfoLoading && <Typography className={styles.helperText}>กำลังดึงข้อมูลหลักสูตรระดับปริญญาโทและสาขาของผู้ใช้งาน...</Typography>}
          ...
          <Box className={styles.formGridTwo}>
            <TextField label="หลักสูตร (พิมพ์ชื่อหลักสูตร/ปีที่ปรับปรุง)" name="curriculumName" value={generalForm.curriculumName} onChange={handleChangeGeneralForm} placeholder="เช่น วิทยาศาสตรมหาบัณฑิต" fullWidth disabled={isFormDisabled} />
        ...
        <Box className={styles.sectionCard}>
          <Typography className={styles.sectionTitle}>ข้อมูลรูปแบบการเรียน</Typography>

          <Box className={styles.studySection}>
            <Box className={styles.optionGroup}>
              <Typography className={styles.optionGroupTitle}>แผน (เฉพาะ ป.โท)</Typography>
              <RadioGroup row name="studyPlan" value={studyForm.studyPlan} onChange={handleChangeStudyForm} className={styles.optionRadioRow}>
                {studyPlanOptions.map((item) => <FormControlLabel key={item.value} value={item.value} control={<Radio />} label={item.label} disabled={isFormDisabled} />)}
              </RadioGroup>
            </Box>
      ...
      <SubjectSelectorModal open={subjectSelectorState.isOpen} onClose={closeSubjectSelector} onSelectSubject={handleSelectSubject} currentMajorName={generalForm.majorName} curriculumLevel="master" />
    </Box>
  )
}
