...
const defaultGeneralForm = { submissionRound: '1', semester: '1', academicYear: '2568', curriculumName: '', majorName: '', doctoralFormType: '1.1', campus: 'chakrabongse' }
...
const getCurriculumNameThai = (curriculum) => {
  return curriculum?.curriculum_name_thai ?? curriculum?.curriculumNameThai ?? curriculum?.curriculum_name_th ?? curriculum?.curriculumNameTh ?? curriculum?.name_thai ?? curriculum?.nameThai ?? curriculum?.name ?? ''
}

const getCurriculumLevelText = (curriculum) => {
  const level = curriculum?.curriculum_level ?? curriculum?.curriculumLevel ?? curriculum?.curriculum_level_name ?? curriculum?.curriculumLevelName ?? curriculum?.degree_level ?? curriculum?.degreeLevel ?? curriculum?.level ?? ''
  if (typeof level === 'object') return normalizeText(level?.name ?? level?.level_name ?? level?.levelName ?? level?.curriculum_level ?? '')
  return normalizeText(level)
}

const isDoctoralCurriculum = (curriculum) => {
  const levelText = getCurriculumLevelText(curriculum).toLowerCase()
  const nameText = getCurriculumNameThai(curriculum).toLowerCase()
  return levelText === 'doctoral' || levelText === 'doctor' || levelText === 'phd' || levelText.includes('doctoral') || levelText.includes('doctor') || levelText.includes('phd') || levelText.includes('ปริญญาเอก') || levelText.includes('เอก') || nameText.includes('ดุษฎีบัณฑิต') || nameText.includes('ปริญญาเอก')
}

const isCurriculumMatchedWithDepartment = (curriculum, departmentId) => {
  const departmentIdText = String(departmentId || '')
  const directDepartmentId = curriculum?.department_id ?? curriculum?.departmentId ?? curriculum?.department?.id ?? curriculum?.major_id ?? curriculum?.majorId ?? null
  if (directDepartmentId && String(directDepartmentId) === departmentIdText) return true
...
function buildInitialPageData(requestData) {
  const documentData = requestData?.documentData || requestData?.document_data || {}
  const defaultApprovalForm = createDefaultApprovalForm()
  const nextGeneralForm = { ...defaultGeneralForm, ...(documentData.generalForm || {}) }

  nextGeneralForm.doctoralFormType = documentData.generalForm?.doctoralFormType || documentData.generalForm?.formType || nextGeneralForm.doctoralFormType
...
function getStatusConfig(status) {
  ...
  if (status === 'pendingApproval' || status === 'pending_approval' || status === 'pending') return { label: 'ส่งเอกสารแล้วกำลังรออนุมัติ', className: styles.statusChipPendingApproval }
...
function CourseOpeningDoctoralPage() {
  ...
  const fetchInitialCurriculumAndDepartment = useCallback(async () => {
    ...
    try {
      ...
      if (!userDepartmentId) {
        ...
        setInitialInfoErrorMessage('ไม่พบรหัสสาขาของผู้ใช้งานปัจจุบัน จึงยังไม่สามารถค้นหาหลักสูตรระดับปริญญาเอกที่ผูกกับสาขาได้')
        return
      }
      ...
      const matchedCurriculum = curriculumDetailList.find((curriculum) => isDoctoralCurriculum(curriculum) && isCurriculumMatchedWithDepartment(curriculum, userDepartmentId))
      ...
      if (!matchedCurriculum) setInitialInfoErrorMessage('พบข้อมูลสาขาของผู้ใช้งานแล้ว แต่ยังไม่พบหลักสูตรระดับปริญญาเอกที่ผูกกับสาขานี้')
    } catch (error) {
      console.error('Error fetching initial doctoral curriculum and department:', error)
      setInitialInfoErrorMessage(getErrorMessage(error, 'ไม่สามารถดึงข้อมูลหลักสูตรระดับปริญญาเอกและสาขาของผู้ใช้งานได้'))
    } finally {
  ...
  const handleChangeGeneralForm = (event) => {
    ...
    setGeneralForm((previousForm) => ({ ...previousForm, [name]: value }))
  }

  const handleChangeYearBlockField = (blockId, fieldName, value) => {
    setYearBlocks((previousBlocks) => previousBlocks.map((block) => (block.id === blockId ? { ...block, [fieldName]: value } : block)))
  }
  ...
  const handleAddSubjectRow = (blockId) => {
    setYearBlocks((previousBlocks) => previousBlocks.map((block) => (block.id === blockId ? { ...block, subjectRows: [...block.subjectRows, createEmptySubjectRow()] } : block)))
  }
  ...
  const handleAddYearBlock = () => {
    ...
    setYearBlocks((previousBlocks) => [...previousBlocks, createYearBlock(nextAvailableYearLevel)])
  }
  ...
  const handleDeleteYearBlock = (blockId) => {
    ...
    setYearBlocks((previousBlocks) => previousBlocks.filter((block) => block.id !== blockId))
  }
  ...
  const handleRemoveResponsiblePerson = (personId) => {
    ...
    setApprovalForm((previousForm) => ({ ...previousForm, responsiblePeople: previousForm.responsiblePeople.filter((person) => person.id !== personId) }))
  }
  ...
  const buildCourseOpeningPayload = () => {
    ...
    return {
      ...
      program_type: generalForm.doctoralFormType,
      doctoral_form_type: generalForm.doctoralFormType,
      education_level: 'doctoral',
      degree_level: 'doctoral',
      study_mode: generalForm.doctoralFormType,
      campus: generalForm.campus,
      target_group: generalForm.doctoralFormType,
  ...
  const validateDraftPayload = (payload) => {
    ...
    if (!payload.doctoral_form_type) return 'กรุณาเลือกประเภทแบบฟอร์มปริญญาเอก'
...
const saveDraftRequest = async ({ returnToViewMode = false } = {}) => {
  ...
  try {
    ...
    console.error('Error saving doctoral course opening draft:', error)
    window.alert(getErrorMessage(error, 'ไม่สามารถบันทึกแบบร่างคำขอเปิดรายวิชาระดับปริญญาเอกได้'))
  } finally {
    setIsSubmittingRequest(false)
  }
}

  const handleSaveDraft = () => saveDraftRequest()
  ...
  return (
    <Box className={styles.page}>
      ...
      <Box className={styles.container}>
        <Box className={styles.pageHeader}>
          <Typography className={styles.pageTitle}>
            {requestData ? 'ดูรายละเอียดการเปิดรายวิชา (ระดับปริญญาเอก)' : 'แบบเปิดรายวิชาประจำภาคการศึกษา (ระดับปริญญาเอก)'}
          </Typography>

          <Typography className={styles.pageDescription}>
            {requestData ? 'หน้านี้ใช้สำหรับตรวจสอบรายละเอียดเอกสารที่บันทึกไว้ โดยสามารถแก้ไขแบบร่างก่อนส่งเอกสารจริงจากหน้ารายการคำขอเปิดรายวิชา' : 'หน้าสำหรับกรอกข้อมูลคำขอเปิดรายวิชาระดับปริญญาเอกและบันทึกเป็นแบบร่างก่อน จากนั้นจึงไปตรวจสอบและกดส่งเอกสารจริงที่หน้ารายการคำขอเปิดรายวิชา'}
          </Typography>
        </Box>
          ...
          <Box className={styles.modeCard}>
            <Box className={styles.modeCardTop}>
              <Box className={styles.modeTextBlock}>
                ...
                <Typography className={styles.modeTitle}>{generalForm.curriculumName || 'ยังไม่ได้ระบุชื่อหลักสูตร'} • สาขา {generalForm.majorName}</Typography>
                <Typography className={styles.modeDescription}>{isViewMode ? 'ในโหมดนี้ข้อมูลทั้งหมดจะถูกล็อกไว้เพื่อให้ตรวจสอบเอกสารได้อย่างเดียว หากต้องการแก้ไขแบบร่างให้กดปุ่มแก้ไขที่ด้านล่างของหน้า' : 'กำลังแก้ไขเอกสารฉบับร่างในหน้าเดิม ข้อมูลทุกส่วนจะถูกปลดล็อกชั่วคราวเพื่อให้แก้ไขและบันทึกได้'}</Typography>
              </Box>
        ...
        <Box className={styles.viewModeHintBox}>
          ...
          <Box>
            <Typography className={styles.viewModeHintTitle}>{isViewMode ? 'กำลังแสดงเอกสารในโหมดดูอย่างเดียว' : isEditMode ? 'กำลังแสดงเอกสารในโหมดแก้ไขแบบร่าง' : 'กำลังแสดงเอกสารในโหมดสร้างแบบร่างใหม่'}</Typography>
            <Typography className={styles.viewModeHintDescription}>{isViewMode ? 'ทุก field และปุ่มภายในเอกสารถูก disabled ไว้ทั้งหมด เพื่อให้ตรวจสอบข้อมูลได้ก่อนตัดสินใจแก้ไข' : isEditMode ? 'สามารถปรับข้อมูลในแบบฟอร์มได้ทั้งหมด เมื่อแก้ไขเสร็จแล้วให้กดบันทึกแบบร่างที่ด้านล่างของหน้า' : 'โหมดนี้ใช้สำหรับกรอกแบบฟอร์มใหม่และบันทึกเป็นแบบร่างก่อนส่งเอกสารจริง'}</Typography>
          </Box>
        </Box>
        ...
        <Box className={styles.sectionCard}>
          ...
          {isInitialInfoLoading && <Typography className={styles.helperText}>กำลังดึงข้อมูลหลักสูตรระดับปริญญาเอกและสาขาของผู้ใช้งาน...</Typography>}
          ...
          <Box className={styles.formGridTwo}>
            <TextField label="หลักสูตร (พิมพ์ชื่อหลักสูตร/ปีที่ปรับปรุง)" name="curriculumName" value={generalForm.curriculumName} onChange={handleChangeGeneralForm} placeholder="เช่น วิทยาศาสตรดุษฎีบัณฑิต" fullWidth disabled={isFormDisabled} />
          ...
          <Box className={styles.formOptionGrid}>
            <Box className={styles.optionGroup}>
              <Typography className={styles.optionGroupTitle}>ประเภทแบบฟอร์ม (ป.เอก)</Typography>
              <RadioGroup row name="doctoralFormType" value={generalForm.doctoralFormType} onChange={handleChangeGeneralForm} className={styles.optionRadioRow}>
                {doctoralFormTypeOptions.map((item) => <FormControlLabel key={item.value} value={item.value} control={<Radio />} label={item.label} disabled={isFormDisabled} />)}
              </RadioGroup>
            </Box>

            <Box className={styles.optionGroup}>
              <Typography className={styles.optionGroupTitle}>ศึกษาที่เขตพื้นที่</Typography>
              <RadioGroup row name="campus" value={generalForm.campus} onChange={handleChangeGeneralForm} className={styles.optionRadioRow}>
            ...
            {yearBlocks.map((block) => (
              <Box key={block.id} className={styles.yearBlock}>
                ...
                <Box className={styles.subjectActionRow}>
                  ...
                  <Typography className={styles.rowCount}>แถวทั้งหมด {block.subjectRows.length} รายการ</Typography>
                </Box>
              </Box>
            ))}
          </Box>
        ...
        {requestData && (
          <Box className={styles.requestActionCard}>
            <Box className={styles.requestActionInfo}>
              ...
              <Typography className={styles.requestActionDescription}>{isViewMode ? 'หากต้องการแก้ไขข้อมูล ให้กดปุ่มแก้ไขเอกสาร ระบบจะปลดล็อก field ทั้งหมดของเอกสารฉบับนี้ในหน้าเดิม' : 'เมื่อบันทึกแบบร่างแล้ว เอกสารจะกลับไปอยู่ในโหมดดูอย่างเดียวอีกครั้ง เพื่อใช้ตรวจสอบก่อนย้อนกลับไปหน้ารายการและกดส่งเอกสาร'}</Typography>
            </Box>

            <Box className={styles.requestActionButtons}>
              {isViewMode && canEditCurrentDocument && <Button variant="contained" startIcon={<EditRoundedIcon />} className={styles.editButton} onClick={handleStartEdit}>แก้ไขเอกสาร</Button>}
      ...
      <SubjectSelectorModal open={subjectSelectorState.isOpen} onClose={closeSubjectSelector} onSelectSubject={handleSelectSubject} currentMajorName={generalForm.majorName} curriculumLevel="doctoral" />
    </Box>
  )
}
